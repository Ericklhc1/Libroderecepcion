...
export function ChatWidget({
  ...
  function invokeFrontiFromGroupSettings() {
    setView('conversation');
    setGroupManageQuery('');
    setBody((current) => {
      if (/(^|\\s)@fronti\\b/i.test(current)) return current;
      const prefix = current && !/\\s$/.test(current) ? `${current} ` : current;
      return `${prefix}@Fronti `.slice(0, CHAT_BODY_MAX);
    });
    window.requestAnimationFrame(() => composerRef.current?.focus());
  }
            ...
            {bootstrap?.frontiEnabled ? (
              <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gold-200">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-petrol-950">Fronti · asistente del chat</p>
                    <p className="mt-0.5 text-xs leading-relaxed text-slate-600">
                      Está disponible en este grupo como actor virtual. No es una persona del directorio ni ocupa un cupo de participante.
                    </p>
                    <button
                      type="button"
                      onClick={invokeFrontiFromGroupSettings}
                      className="mt-2 inline-flex items-center gap-1.5 rounded-lg bg-petrol-900 px-3 py-2 text-xs font-semibold text-white hover:bg-petrol-800"
                    >
